...
def test_the_archive_carries_every_directory_the_instance_grew(backup_db, instance):
    ...
    assert manifest["excluded"] == [
        "backups/", "exports/", "lesson-builds/", "*.pre-restore-*",
        ".restore-tmp-*", "activity.sqlite*",
    ]


def test_the_reconstructible_package_workspace_is_declared_not_half_archived(
    backup_db, instance, tmp_path
):
    ...
    already incomplete list. The tree is derived state a build reinstalls, so it
    is declared reconstructible instead: left out whole, named in `excluded`, and
    not recreated by a restore.
    """
    tree = instance / "lesson-builds" / "invented-lesson" / "node_modules"
    ...
    assert not any(
        name.startswith("lesson-builds/") for name in manifest["instance_files"]
    ), "a package tree cannot be carried faithfully, so it is not carried in half"
    assert "lesson-builds/" in manifest["excluded"]
    assert "lessons/demo-slug/lesson.json" in manifest["instance_files"], (
        "and the rest of the instance is archived exactly as before"
    )

    target = tmp_path / "recovered"
    result = backup_db.restore(manifest_path, target)

    assert "lesson-builds" not in result["restored"]
    assert not (target / "lesson-builds").exists(), (
        "the next build of a lesson reinstalls its workspace; a restore that "
        "made the directory would be claiming it had recovered one"
    )
...
def test_a_renamed_database_is_still_excluded_from_the_archive(
    ...
    assert manifest["excluded"] == [
        "backups/", "exports/", "lesson-builds/", "*.pre-restore-*",
        ".restore-tmp-*", "activity.sqlite*", "ledger.sqlite*",
    ], (
