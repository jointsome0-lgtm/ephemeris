...
- [x] 2026-07-21 — de51c00 — `app/terminal.py`, `app/sandbox.py`, `verify.py` —
  phase E session E2 classifies every session-create request with a `lesson`
  parameter as `lesson-agent`, spawns it through the E1 sandbox launcher with
  visible fail-closed refusal, records immutable creation-time
  ...
  direct repository shell path. Drained 2026-07-21 →
