...
is `9c85795^..862d3b3`, three commits from `9c85795` through `862d3b3`.

**Starting HEAD:** `862d3b34cc83b027dd3a15be28c9ea6cac92d5bd` on
`fix/42-e4-terminal-surfaces`. `git status --short --branch` showed only
`## fix/42-e4-terminal-surfaces...origin/fix/42-e4-terminal-surfaces`, and
`git status --porcelain=v1 --untracked-files=all` was empty before review work,
so the worktree was clean and the starting branch matched its refreshed remote.

**Fix target:** after refreshing `origin`, neither scoped commit `9c85795` nor
starting head `862d3b3` was an ancestor of local `main` or current
`origin/main`; `main...HEAD` contained three commits only on the E4 side, and
only `fix/42-e4-terminal-surfaces` contained the starting head. This is a
not-yet-merged branch drain, so every drain fix belongs on
`fix/42-e4-terminal-surfaces`, not directly on `main`.
...
closed on the E4 branch and the exact fixed tree is re-verified. Wider,
proxy-adjacent, multi-user, or runner deployment remains NO independently.**

## CLOSING ADDENDUM — fix commit `0ac39bd` (cycle 1 of 10)
...
Fresh review of `0ac39bd` found no new Critical, High, Medium, Low, or Info
...
- Starting HEAD `862d3b3`: `python verify.py` — **636 passed, 0 failed**;
