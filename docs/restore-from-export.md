...
| `focus_sessions` | Partially | The event carries session ID, mode, seconds, target length, the note and the target ids (`app/services/focus.py`, `_write_session`); the row's authoritative dates and timestamps are absent. The note is therefore *in the export but not replayed* — recoverable by hand from the stream, unlike a field that was never written down. Pre-#75 events name the retired `pomo`/`stopwatch` modes; nothing replays them into the table, so they stay restorable. |
