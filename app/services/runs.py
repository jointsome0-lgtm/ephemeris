...
from . import artifacts, lessons, projection
...
def _write_state(uid: str, st: os.stat_result) -> None:
    ...
    data = (json.dumps({
        ...
        "lesson_uid": uid,
        "file": projection.file_seal(st),
    }, sort_keys=True, separators=(",", ":")) + "\n").encode("ascii")
    dir_fd = os.open(PROJECTION_STATE_DIR, projection.DIRECTORY_FLAGS)
    try:
        projection.publish(dir_fd, f"{uid}.json", data, prefix=".run-state-")
    finally:
        os.close(dir_fd)
...
def _publish_whole(
    ...
    seal is written for the inode whose bytes were actually synced; the
    publisher refuses if the name was swapped under it, leaving no seal, so
    the next run starts over.
    """
    published = projection.publish(
        dir_fd, PROJECTION_NAME, payload, prefix=".runs-", expect=expect,
    )
    if published is None:
        return False
    _write_state(uid, published)
    return True
...
def _restart_projection(dir_fd: int, uid: str, line: bytes) -> None:
    ...
    """
    projection.move_aside(dir_fd, PROJECTION_NAME, retire_regular=True)
...
def _read_sealed_tail(dir_fd: int, state: dict, budget: int) -> bytes | None:
    ...
    """
    try:
        fd = os.open(PROJECTION_NAME, projection.READ_FLAGS, dir_fd=dir_fd)
    except OSError:
        return None
    try:
        st = os.fstat(fd)
        if not projection.seal_matches(st, state["file"]):
            return None
...
def _publish_projection_line(job: runner.RunnerJob, uid: str, line: bytes) -> None:
    ...
    dir_fd = os.open(bundle_dir, projection.DIRECTORY_FLAGS)
    try:
...
def retire_foreign_projection(lesson: dict) -> bool:
    ...
    """
    try:
        uid = projection.safe_uid(lesson)
        bundle_dir = lessons._lesson_dir(lesson["slug"])
        with projection.file_lock(PROJECTION_STATE_DIR, lesson, blocking=True):
            dir_fd = os.open(bundle_dir, projection.DIRECTORY_FLAGS)
            try:
                try:
                    st = os.stat(
                    ...
                    return True
                state = _read_state(uid)
                if state is not None and projection.seal_matches(st, state["file"]):
                    return True
                projection.move_aside(dir_fd, PROJECTION_NAME, retire_regular=True)
                os.fsync(dir_fd)
                return True
...
def _project_finish(job: runner.RunnerJob, record: dict) -> None:
    ...
    uid = projection.safe_uid(lesson)
    with projection.file_lock(PROJECTION_STATE_DIR, lesson, blocking=True):
        _publish_projection_line(job, uid, line)
