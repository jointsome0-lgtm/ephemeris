...
if (frame && frame.dataset["metaUrl"] && frame.getAttribute("src")) {
    ...
    const deriveRunIdempotencyKey = async (requestId, blockId, content) => {
    ...
    const postAttempt = async (boundPort, gen, requestId, questionId, answer) => {
        ...
        try {
            ...
            if (result === "recorded") {
                 ...
                 * learner either. */
                if (asked)
                    toast("question sent to the tutor");
                else
...
if (recordPanel && recordCountsUrl && recordKey) {
    ...
    const pollCounts = async () => {
        ...
        try {
            ...
            const unread = typeof counts.unread === "number" && Number.isFinite(counts.unread)
                ? counts.unread
                : 0;
            showUnread(unread);
