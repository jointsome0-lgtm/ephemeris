...
if (frame && frame.dataset["metaUrl"] && frame.getAttribute("src")) {
  ...
  const deriveRunIdempotencyKey = async (
    ...
    const digest = await crypto.subtle.digest("SHA-256", UTF8.encode(JSON.stringify([
  ...
  const finishReady = async (
    ...
    capabilities = [];
    if (want.includes("attempts")) capabilities.push("attempts");
    ...
    if (
      ...
      const questions = recordSnapshot.questions.filter(
...
interface RecordCounts {
  ok?: unknown;
  attempts?: unknown;
  /* Questions to the tutor, counted apart from attempts (#136). */
  questions?: unknown;
  assessments?: unknown;
  verdicts?: unknown;
  unread?: unknown;
...
if (recordPanel && recordCountsUrl && recordKey) {
  ...
  if (!readSeen()) {
    ...
    writeSeen(
      (recordPanel.open && recordPanel.dataset["recordCursor"]) || RECORD_ZERO_CURSOR,
    );
  }
  ...
  const showAndAcknowledge = async (): Promise<void> => {
    const shown = await guardedRefresh();
  ...
  recordPanel.addEventListener("toggle", () => {
    ...
    if (!newest || readSeen() >= newest) return;
    void showAndAcknowledge();
  });
