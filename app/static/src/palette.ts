/* GENERATED-SOURCE NOTICE: app/static/palette.js is emitted from this file by
 * `bun run build` (tsc, issue #42) and committed so deploy stays zero-build.
 * Edit THIS file and re-emit; never edit the .js by hand. */

// palette.ts — command palette (⌘K / Ctrl K). Framework-free progressive
...
// whose shape is declared there — this file is a classic deferred script too, so
// no top-level import/export.
(() => {
  "use strict";

  /** One row of GET /palette.json, before this file tags it with its category. */
  ...
  type PaletteRow = PaletteEntry & { tag: string };
  /** The whole index: one array per category key in CATS. */
  type PaletteIndex = Record<string, PaletteEntry[]>;

  // category key in the JSON -> short tag shown on each row, in display order
  const CATS: ReadonlyArray<readonly [string, string]> =
               [["views", "Go"], ["lists", "List"], ["habits", "Habit"],
                ["lessons", "Learn"], ["actions", "Do"]];

  let cache: PaletteIndex | null = null, cacheAt = 0, rows: PaletteRow[] = [], sel = 0, open = false;
  // Assigned by build(), which openPalette() runs before anything else can
  // reach them — hence the definite-assignment marks rather than a null check
  // in every function.
  let overlay!: HTMLDivElement, input!: HTMLInputElement, listEl!: HTMLUListElement;
  ...
  function flat(d: PaletteIndex): PaletteRow[] {
  ...
  function esc(v: unknown): string {
    // The regex matches only the four keys below, so the fallback never fires;
    // an unmatched character standing for itself is the honest default anyway.
  ...
  function build(): void {
    ...
    overlay.innerHTML =
      ...
      "</div>";
    // The markup above was just written, so neither lookup can miss.
    input = overlay.querySelector<HTMLInputElement>(".pal-input")!;
    listEl = overlay.querySelector<HTMLUListElement>(".pal-list")!;
    overlay.addEventListener("mousedown", (e) => { if (e.target === overlay) close(); });
    input.addEventListener("input", () => render(input.value.trim()));
    input.addEventListener("keydown", onKey);
    listEl.addEventListener("click", (e) => {
      const li = (e.target as Element).closest<HTMLElement>("[data-i]");
      if (li) activate(rows[Number(li.dataset["i"])]);
    });
  ...
  function activate(it: PaletteRow | undefined): void {
    if (!it) return;
    close();
    if (it.href) { window.location.href = it.href; return; }
    const ui: AlUI = window.alUI || {};
