...
(() => {
    ...
    function build() {
        ...
        listEl.addEventListener("click", (e) => {
            ...
            if (li)
                activate(rows[Number(li.dataset["i"])]);
        });
    ...
    function render(q) {
        ...
        listEl.innerHTML = list.length
            ? list.map((it, i) => '<li class="pal-row" role="option" data-i="' + i + '" id="pal-o-' + i + '">' +
    ...
    function activate(it) {
        ...
        if (it.shortcut === "n" && ui.newTask)
            ui.newTask();
        else if (it.shortcut === "f" && ui.toggleTimer)
            ui.toggleTimer();
        else if (it.shortcut === "t" && ui.toggleTheme)
            ui.toggleTheme();
        else if (it.shortcut === "?" && ui.showHints)
            ui.showHints();
    }
    async function openPalette() {
        ...
        await load();
        if (open)
            render(""); // refresh once the index lands
    }
    function close() {
        if (!open)
            return;
