...
(() => {
    ...
    (() => {
        ...
        month.addEventListener("dragstart", (e) => {
            ...
            const chip = target && target.closest('.cm-event.ev[draggable="true"]');
            if (!chip)
                return;
            dragged = chip;
            ...
            try {
                dt.setData("text/plain", chip.dataset["evId"] ?? "");
            }
            catch (_) { /* Safari */ }
        });
        ...
        month.querySelectorAll(".cm-cell[data-date]").forEach((cell) => {
            cell.addEventListener("dragover", (e) => {
                if (!dragged || cell.dataset["date"] === dragged.dataset["evDate"])
                    return;
                e.preventDefault();
                if (e.dataTransfer)
            ...
            cell.addEventListener("drop", async (e) => {
                ...
                const res = await post("/calendar/events/" + (chip.dataset["evId"] ?? "") + "/move", { date: newDate, return_to: "/calendar" });
                if (!res.ok) {
