...
(() => {
    ...
    if (!board)
        return;
    const toast = (m) => { if (window.alUI && window.alUI.toast)
        window.alUI.toast(m); };
    ...
    const ORDER = Array.from(board.querySelectorAll(".bcol"))
    ...
    function place(card, status) {
        ...
        if (status === "done")
            ...
            body.insertBefore(card, body.querySelector(".bcol-empty"));
    ...
    async function move(card, status) {
        ...
        if (!id || status === card.dataset["status"])
            return;
        ...
        let target = status;
        for (;;) {
            const res = await post("/tasks/" + id + "/status", { status: target, return_to: location.pathname });
            if (!res.ok) {
                pending.delete(id);
    ...
    board.querySelectorAll(".bcol-body[data-drop]").forEach((body) => {
        body.addEventListener("dragover", (e) => {
            if (!dragged || body.dataset["drop"] === dragged.dataset["status"])
                return;
            e.preventDefault();
            if (e.dataTransfer)
