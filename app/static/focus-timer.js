...
(() => {
    ...
    const els = {
        ...
        custom: $("timer-custom"),
        target: $("timer-target"),
        note: $("timer-note"), error: $("timer-error"),
    ...
    let ticker;
    let poller; // the resync loop; its period follows `run`
    ...
    async function sync(keepError) {
        if (busy)
            return false; // a write is in flight; its answer is the newer one
        const ticket = ++issueSeq;
        try {
    ...
    async function startTimer() {
        ...
        // just promised.
        if (targetsLoading)
            await targetsLoading;
        if (busy || run)
            return; // a second click got here first while we waited
        ...
        if (mode === "countdown")
            params["target_seconds"] = String(Math.round(minutes * 60));
        ...
        if (els.note.value.trim())
            params["note"] = els.note.value.trim();
    ...
    async function autoFinish() {
        ...
        if (done && done.recorded)
            toast("Focus recorded · " + done.recorded.duration_label);
    }
    ...
    els.custom.addEventListener("input", () => {
        ...
        if (!raw) {
            minutes = preset;
            els.lengths.querySelectorAll("button").forEach((x) => x.classList.toggle("on", parseInt(x.dataset["minutes"] ?? "", 10) === preset));
        }
        else {
            const v = Number(raw);
    ...
    $("timer-min").addEventListener("click", () => setOpen(true, !drawer.classList.contains("minimized")));
    document.querySelectorAll(".timer-toggle").forEach((b) => b.addEventListener("click", toggleTimer));
