...
- Codes with no fixture here need runtime context and are synthesized in C3
  tests instead: `manifest-too-large` (an oversized file),
  `identity-mismatch` / `stale-metadata` (need a DB row to disagree with).
